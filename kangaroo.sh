...
validate() {
    ...
    # Validar TARGET_PORTS — no vacio, solo digitos y comas
    if [[ -z "$TARGET_PORTS" ]]; then
        error "TARGET_PORTS no puede estar vacio."
        exit 1
    fi
    if ! echo "$TARGET_PORTS" | grep -qE '^[0-9]+(,[0-9]+)*$'; then
        error "TARGET_PORTS debe ser lista de puertos separados por coma: ej. 22,80,443"
        exit 1
    fi
    ...
    # Nombre del .rc temporal de recon (robusto ante cualquier extension)
    SCAN_OUTPUT_RC="$(dirname "$OUTPUT_RC")/$(basename "$OUTPUT_RC" .rc)_recon.rc"
...
EOF
    ...
    # Verificar que el directorio del log es escribible
    local log_dir
    log_dir=$(dirname "$SCAN_LOG")
    if [[ ! -w "$log_dir" ]]; then
        warn "Sin permisos de escritura en $log_dir — recon sin log persistente."
        msfconsole -q -r "$SCAN_OUTPUT_RC" 2>&1 | grep "TCP OPEN" || true
        return 0
    fi
    ...
    if [[ ! -s "$HOSTS_FILE" ]]; then
        ...
        rm -f "$SCAN_OUTPUT_RC" "$HOSTS_FILE" "$SCAN_LOG"
        return 0
    fi
    ...
    local choice
    while true; do
        if ! read -r -t 60 -p "  Opcion [0-$total]: " choice 2>/dev/null; then
            warn "Timeout — continuando sin target fijo."
            choice=0
            break
        fi
...
# ── 5. Instrucciones de uso (print_status es el comando correcto en msfconsole) ───
print_status ""
print_good   "Kangaroo — Pivoting activo"
print_status "Red interna : $PIVOT_NETWORK"
print_status "SOCKS$SOCKS_VERSION     : 127.0.0.1:$SOCKS_PORT"
$([ -n "$TARGET_HOST" ] && echo "print_good \"Target      : $TARGET_HOST\"" || echo "print_status \"Target      : usa proxychains contra cualquier host\"")
print_status ""
print_status "proxychains nmap -sT -Pn -p $TARGET_PORTS ${TARGET_HOST:-<TARGET_IP>}"
print_status "proxychains curl -v http://${TARGET_HOST:-<TARGET_IP>}"
print_status "proxychains ssh user@${TARGET_HOST:-<TARGET_IP>}"
print_status ""
EOF
